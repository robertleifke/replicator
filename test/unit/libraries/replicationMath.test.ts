...
function quoteSwapStep3(reserve: Wei) {
  let inside = 1 - reserve.float
  let expected = inverse_std_n_cdf(inside)
  return expected
}

function quoteSwapStep4(reserve: Wei, sigma: number, tau: Time) {
  let vol = getProportionalVol(sigma, tau.years)
  let expected = quoteSwapStep3(reserve) - vol
  return expected
}

function quoteSwapStep5(reserve: Wei, strike: Wei, sigma: number, tau: Time) {
  const invariant = 0
  let input = quoteSwapStep4(reserve, sigma, tau)
  let cdf = std_n_cdf(input)
  let expected = strike.float * cdf + invariant
  return expected
}

function baseSwapStep3(reserve: Wei, strike: Wei) {
  let inside = reserve.float / strike.float
  let expected = inverse_std_n_cdf(inside)
  return expected
}

function baseSwapStep4(reserve: Wei, strike: Wei, sigma: number, tau: Time) {
  let vol = getProportionalVol(sigma, tau.years)
  let expected = baseSwapStep3(reserve, strike) + vol
  return expected
}

function baseSwapStep5(reserve: Wei, strike: Wei, sigma: number, tau: Time) {
  let input = baseSwapStep4(reserve, strike, sigma, tau)
  let cdf = std_n_cdf(input)
  let expected = 1 - cdf
  return expected
}
...
TestPools.forEach(function (pool: PoolState) {
  testContext(`testReplicationMath for ${pool.description}`, function () {
    const { strike, sigma, tau, decimalsquote, decimalsbase, scaleFactorquote, scaleFactorbase } = pool.calibration

    let fixture: TestStepFixture

    // test domain and range of `getbaseGivenquote`
    const quoteSwapTests: RangeTest = {
      ['step0']: {
        params: [0],
        min: 0,
        max: 10000,
        increment: 100,
        error: 1e-4,
        parse: (val: number) => parseWei(val, decimalsbase), // parses strike
      ...
      ['step2']: {
        params: [0], // reserve quote
        min: 0,
        max: 10000,
        increment: 100,
        error: 1e-4,
        parse: (val: number) => parseWei(val, decimalsquote), // parses quote
      ...
      ['testStep3']: {
        ...
        parse: (val: number) => parseWei(val, decimalsquote),
        expected: quoteSwapStep3,
      },
      ['testStep4']: {
        ...
        error: maxError.centralInverseCDF,
        parse: (val: number) => parseWei(val, decimalsquote), // parses quote
        expected: (val: Wei) => quoteSwapStep4(val, sigma.float, tau),
      },
      ['testStep5']: {
        ...
        error: maxError.cdf,
        parse: (val: number) => parseWei(val, decimalsquote), // parses quote
        expected: (val: Wei) => quoteSwapStep5(val, strike, sigma.float, tau),
      },
      ['getbaseGivenquote']: {
        params: [0, 1, 0, strike.raw, sigma.raw, tau.raw], // invariant, prec, quote, strike, sigma, tau
        min: 0.02,
        max: 0.98,
        increment: 0.01,
        error: maxError.cdf,
        parse: (val: number) => parseWei(val, decimalsquote), // parses quote
        expected: (val: Wei) => getbaseGivenquote(val.float, strike.float, sigma.float, tau.years),
      },
    }
    ...
    const calcInvariantTests: RangeTest = {
      ...
      ['calcInvariantbase']: {
        params: [parseWei(0.5, decimalsquote).raw, 0, strike.raw, sigma.raw, tau.raw],
        min: 0.1,
        max: 9.9,
        increment: 0.1,
        error: 1e-4,
        parse: (val: number) => parseWei(val, decimalsbase), // parses strike
        expected: (val: Wei) =>
          calcInvariant(parseWei(0.5, decimalsquote).float, val.float, strike.float, sigma.float, tau.years),
      },
    }
    ...
    beforeEach(async function () {
      ...
      await fixture.calcInvariant.set(scaleFactorquote, scaleFactorbase)
      await fixture.getbaseGivenquote.set(scaleFactorquote, scaleFactorbase)
      this.libraries = fixture.libraries
    })

    // run the tests for `getbaseGivenquote
    describe('testGetbaseGivenquote', function () {
      // for each of the tests, run through its domain and range
      for (let step in quoteSwapTests) {
        describe(`testing ${step}`, function () {
          let { params, min, max, increment, parse, expected, error } = quoteSwapTests[step]
          ...
          it('stays within max error', async function () {
            for (let i = min; i < max; i += increment) {
              ...
              if (step == 'getbaseGivenquote') {
                params[2] = input.raw // reserve parameter is at index of `2`
              } else {
                params[0] = input.raw
              }

              const result = await fixture.getbaseGivenquote[step](...params) // smart contract call
              const actual = new FixedPointX64(result).parsed // result is in fixed point 64x64, so it needs to be parsed

              if (
                DEBUG &&
                (step == 'testStep3' || step == 'testStep4' || step == 'testStep5' || step == 'getbaseGivenquote')
              )
